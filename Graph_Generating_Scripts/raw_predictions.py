...
Author: Rekkab Gill
Date: October 7 2022
Details: A graph to visualize the chromosomal compartment predictions and targets of the ABCnet algorithm by Matthew Kirchhoff. 
"""
...
ax[0].set_ylabel('PC1', fontsize = 32,labelpad= 28)
ax[0].set_ylim([0,1])
...
ax0_blue = ax[0].fill_between(df1_indices,0.5,dataframe1['predictions'],where = dataframe1['predictions'] > 0.5,interpolate = True,facecolor = 'Navy')
ax0_gold = ax[0].fill_between(df1_indices,0.5,dataframe1['predictions'],where = dataframe1['predictions'] < 0.5,interpolate = True,facecolor = 'Gold')
...
ax[1].set_ylim([0,1])
ax[1].set_ylabel('PC1', fontsize = 32,labelpad= 28)
...
ax[1].fill_between(df1_indices,0.5,dataframe1['groundtruth'],where = dataframe1['groundtruth'] > 0.5,interpolate = True,facecolor = 'Navy')
ax[1].fill_between(df1_indices,0.5,dataframe1['groundtruth'],where = dataframe1['groundtruth'] < 0.5,interpolate = True,facecolor = 'Gold')
ax[1].axhline(y=0.5, color = 'black', linewidth = 0.5)
ax[1].margins(0)
...
#ax[2].set_xlabel('chrom1:1-248956422', fontsize = 32,labelpad= 12) #HUMAN
ax[2].set_ylabel('Difference', fontsize = 32,labelpad= 8)
ax2_max_range = np.max([np.max(dataframe1['difference']), np.min(dataframe1['difference']) * (-1)]) # np.max returns the maximum along a given axis 
ax[2].set_ylim([ax2_max_range * -1.1, ax2_max_range * 1.1])
